...
(function (B) {
    ...
    B.color.make = function (F, E, C, D) {
        ...
        G.toString = function () {
            if (G.a >= 1)return "rgb(" + [G.r, G.g, G.b].join(",") + ")"; else return "rgba(" + [G.r, G.g, G.b, G.a].join(",") + ")"
        };
    ...
    B.color.parse = function (F) {
        ...
        if (E = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(F))return C(parseInt(E[1],
        ...
        if (E = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(F))return C(parseFloat(E[1]) *
...
(function ($) {
    function Plot(placeholder, data_, options_, plugins) {
        var series = [], options = {
            ...
            processOptions: [], processRawData: [],
            processDatapoints: [], drawSeries: [], draw: [], bindEvents: [], drawOverlay: [], shutdown: []
        }, plot = this;
        ...
        plot.pointOffset = function (point) {
            return {
                left: parseInt(xaxes[axisNumber(point,
                        "x") - 1].p2c(+point.x) + plotOffset.left),
        ...
        function parseOptions(opts) {
            ...
            if (options.yaxis.noTicks && options.yaxis.ticks == null)options.yaxis.ticks =
        ...
        function setupTickGeneration(axis) {
            ...
            var noTicks;
            if (typeof opts.ticks == "number" && opts.ticks > 0)noTicks = opts.ticks; else noTicks = 0.3 * Math.sqrt(axis.direction == "x" ? canvasWidth : canvasHeight);
            ...
            if (opts.mode == "time") {
                ...
                var minSize = 0;
                if (opts.minTickSize !=
                    null)if (typeof opts.tickSize == "number")minSize = opts.tickSize; else minSize = opts.minTickSize[0] * timeUnitSize[opts.minTickSize[1]];
                ...
                if (unit == "year") {
                    ...
                    if (norm < 1.5)size = 1; else if (norm < 3)size = 2; else if (norm <
                        7.5)size = 5; else size = 10;
                    size *= magn
                }
                ...
                generator = function (axis) {
                    ...
                    do {
                        ...
                        if (unit == "month")if (tickSize < 1) {
                            ...
                            carry = d.getUTCHours();
                            d.setUTCHours(0)
                        } else d.setUTCMonth(d.getUTCMonth() + tickSize); else if (unit == "year")d.setUTCFullYear(d.getUTCFullYear() + tickSize); else d.setTime(v + step)
                ...
                formatter = function (v, axis) {
                    ...
                    if (t < timeUnitSize.minute)fmt = "%h:%M:%S" + suffix; else if (t < timeUnitSize.day)if (span < 2 * timeUnitSize.day)fmt = "%h:%M" + suffix; else fmt = "%b %d %h:%M" + suffix; else if (t < timeUnitSize.month)fmt = "%b %d"; else if (t < timeUnitSize.year)if (span < timeUnitSize.year)fmt = "%b"; else fmt = "%b %y"; else fmt = "%y";
        ...
        function setTicks(axis) {
            ...
            if (oticks == null || typeof oticks == "number" && oticks > 0)ticks = axis.tickGenerator(axis); else if (oticks)if ($.isFunction(oticks))ticks = oticks({
                min: axis.min,
                max: axis.max
            }); else ticks = oticks;
            var i, v;
        ...
        function drawGrid() {
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (ctx.lineWidth == 1)if (axis.direction == "x")x = Math.floor(x) + 0.5; else y = Math.floor(y) + 0.5;
                    ctx.moveTo(x,
                        y);
                    ctx.lineTo(x + xoff, y + yoff)
                }
        ...
        function drawSeriesLines(series) {
            ...
            function plotLineArea(datapoints, axisx, axisy) {
                ...
                while (true) {
                    ...
                    if (x1 != x1old)ctx.lineTo(axisx.p2c(x1old),
                        axisy.p2c(y1));
        ...
        function drawBar(x, y, b, barLeft, barRight, offset, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
            ...
            if (lineWidth > 0 && (drawLeft || drawRight || drawTop || drawBottom)) {
                ...
                if (drawBottom)c.lineTo(left, bottom + offset); else c.moveTo(left,
                    bottom + offset);
                c.stroke()
            }
        }
        ...
        function getFillStyle(filloptions, seriesColor, bottom, top) {
            ...
            if (!fill)return null;
            if (filloptions.fillColor)return getColorOrGradient(filloptions.fillColor,
        ...
        function findNearbyItem(mouseX, mouseY, seriesFilter) {
            ...
            for (i = series.length - 1; i >= 0; --i) {
                ...
                if (s.lines.show || s.points.show)for (j =
                ...
                if (s.bars.show && !item) {
                    ...
                    for (j = 0; j < points.length; j += ps) {
                        ...
                        if (series[i].bars.horizontal ?
                            mx <= Math.max(b, x) && mx >= Math.min(b, x) && my >= y + barLeft && my <= y + barRight : mx >= x + barLeft && mx <= x + barRight && my >= Math.min(b, y) && my <= Math.max(b, y))item = [i, j / ps]
                    }
                }
            }
            if (item) {
        ...
        function onMouseLeave(e) {
            if (options.grid.hoverable)triggerClickHoverEvent("plothover",
                e, function (s) {
                    return false
                })
        }
        ...
        function triggerRedrawOverlay() {
            if (!redrawTimeout)redrawTimeout =
    ...
    $.plot.formatDate = function (d, fmt, monthNames) {
        ...
        if (fmt.search(/%p|%P/) != -1)if (hours >
            12)hours = hours - 12; else if (hours == 0)hours = 12;
